#!/usr/bin/env python3
"""Task 1.4: explain the final augmented AI-image detector with Grad-CAM.

Run from the solution directory with, for example:

    python explain.py --timeout_seconds 600

The script is CPU-only, reads the two validation parquet splits without changing
them, and writes all generated material below ``artifacts/task04``.
"""
...
# Keep plotting caches with the other Task 1.4 outputs in restricted runners.
...
def clean_previous_outputs(task_dir: Path, figures_dir: Path) -> None:
    """Remove only files generated by an earlier Task 1.4 run."""
    generated_names = [
...
def set_deterministic(seed: int) -> None:
    """Configure deterministic, CPU-only execution."""
...
def evaluation_transform() -> transforms.Compose:
    """Return the exact deterministic Task 1.3 inference transform."""
...
def resolve_split_dir(name: str, script_dir: Path) -> Path:
    """Prefer project-local data, retaining the Task 1.3 resolver as fallback."""
...
def load_final_model(checkpoint: Path) -> StrongerCNNDetector:
    """Build the final Task 1.3 architecture and load its saved CPU weights."""
...
def load_saved_threshold(path: Path) -> float:
    """Load, rather than re-estimate, the calibrated Task 1.3 threshold."""
    if not path.is_file():
        raise FileNotFoundError(
...
def deterministic_indices(length: int, maximum: int | None) -> list[int]:
    """Choose reproducible, split-wide indices when a sample cap is requested."""
...
def evaluate_split(
    ...
    """Evaluate one split and periodically persist partial results."""
...
def select_examples(results: pd.DataFrame, maximum: int) -> pd.DataFrame:
    """Deterministically select confident, erroneous, and uncertain examples."""
    ...
    if len(selected) < maximum:
        ...
        # Prefer one uncertain example from each split when capacity permits.
...
def original_image(dataset: ParquetCalibDataset, index: int) -> Image.Image:
    """Decode and perform the same initial 224px bicubic resize as the project dataset."""
...
def preprocess_display_image(image: Image.Image) -> torch.Tensor:
    """Apply Task 1.3 inference preprocessing to an already decoded image."""
